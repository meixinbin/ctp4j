...
public class MarketDataDaoImpl implements MarketDataDao{
	...
	public List<MarketData> getList(String instrumentId,long start,long end){
		return mongoTemplate.find(new Query().addCriteria(Criteria.where("instrumentID").is(instrumentId).and("id").gte(start).lt(end)).with(new Sort(Sort.Direction.ASC, "id")), MarketData.class);
	}
